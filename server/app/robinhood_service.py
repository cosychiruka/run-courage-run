...
async def get_robinhood_crypto_stats() -> List[Dict[str, Any]]:
    ...
    Queries CoinGecko's live /search/trending API.
    """
    ...
    # Use ONLY the live trending tokens returned by CoinGecko if available; otherwise use default fallback list
    if trending_ids:
    ...
    # 2. Fetch market stats (price, 24h %, volume, market cap) for the trending tokens
...
def _get_fallback_robinhood_stats() -> List[Dict[str, Any]]:
    ...
    return [
        {"symbol": "$PEPE", "name": "Pepe", "price": 0.0000098, "change_24h": 14.2, "platform": "Robinhood & Trending Crypto", "is_trending": True},
        {"symbol": "$DOGE", "name": "Dogecoin", "price": 0.125, "change_24h": 6.4, "platform": "Robinhood & Trending Crypto", "is_trending": True},
        {"symbol": "$SOL", "name": "Solana", "price": 148.5, "change_24h": 5.8, "platform": "Robinhood & Trending Crypto", "is_trending": True},
        {"symbol": "$SHIB", "name": "Shiba Inu", "price": 0.0000185, "change_24h": 4.1, "platform": "Robinhood & Trending Crypto", "is_trending": True},
        {"symbol": "$SUI", "name": "Sui", "price": 1.05, "change_24h": 11.5, "platform": "Robinhood & Trending Crypto", "is_trending": True},
        {"symbol": "$WIF", "name": "dogwifhat", "price": 1.62, "change_24h": 9.7, "platform": "Robinhood & Trending Crypto", "is_trending": True},
        {"symbol": "$BONK", "name": "Bonk", "price": 0.000021, "change_24h": 7.3, "platform": "Robinhood & Trending Crypto", "is_trending": True},
    ]
