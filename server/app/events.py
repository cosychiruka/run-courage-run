...
async def trigger_realtime_art(prompt: str, news_image_url: str = None, game_context: str = None):
    """Called by Orchestrator on high-priority events (now supports news image)."""
    from app.image_gen import create_courage_art_realtime
    url = await create_courage_art_realtime(prompt, news_image_url=news_image_url, game_context=game_context)
    if url:
